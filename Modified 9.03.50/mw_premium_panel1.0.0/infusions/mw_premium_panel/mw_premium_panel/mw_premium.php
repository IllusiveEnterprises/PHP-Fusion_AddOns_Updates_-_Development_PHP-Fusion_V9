...
if (isset($_POST['activate'])) {
	...
	if ((($premiumsettings['status'] == '0') || ($premiumsettings['status'] == '')) && score_account_stand() >= $packdata['pack_price']) {
		...
		$message1 = $locale['MWP_a060']."<br />".$locale['MWP_a061']."<br />".$locale['MWP_a062'].showdate("%d.%m.%Y %H:%M:%S", $messagesettings['time_off']).$locale['MWP_a063'];
		$message2 = $locale['MWP_a064'].$userdata['user_name'].$locale['MWP_a065']."<br />".$locale['MWP_a062'].showdate("%d.%m.%Y %H:%M:%S", $messagesettings['time_off']).$locale['MWP_a063'];
		$usermessage = dbquery("INSERT INTO ".DB_MESSAGES." (message_to, message_from, message_user, message_subject, message_message, message_smileys, message_read, message_datestamp, message_folder) VALUES('".$userdata['user_id']."','1','".$userdata['user_id']."','Premium-Account','".$message1."','y','0','".time()."','0')");
		$adminmessage = dbquery("INSERT INTO ".DB_MESSAGES." (message_to, message_from, message_user, message_subject, message_message, message_smileys, message_read, message_datestamp, message_folder) VALUES('1','".$userdata['user_id']."','1','Premium-Account','".$message2."','y','0','".time()."','0')");
...
opentable($locale['MWP_a050']);
echo "<fieldset>";
echo "<legend>".(($premiumsettings['status'] == '') || ($premiumsettings['status'] == '0') ? $locale['MWP_a051'] : $locale['MWP_a052'])."</legend>";
echo "<form method='POST' action='mw_premium.php'>";
echo "<table border='0' align='center' cellpadding='5' cellspacing='0'>";
echo "<tr><td align='center' colspan='3'><u>".$locale['MWP_a053'].":</u></td></tr>";
echo "<tr><th>".$locale['MWP_a054']."</th><th>".$locale['MWP_a055']."</th></tr>";
...
echo "</tr>";
echo "<tr><th colspan='3'><br><font size='3' color='".(score_account_stand() >= $packdata['pack_price'] ? "#00FF00" : "red")."'>".$locale['MWP_a057'].$score_settings['set_units'].$locale['MWP_a058'].": ".score_account_stand()." <img src='".INFUSIONS."mw_premium_panel/images/score.png' width='20px' height='20px'></font><br><br>";
if (score_account_stand() >=$packdata['pack_price']) {
	echo "<input type='hidden' value='".$packdata['pack_id']."' name='id'>";
	echo "<input type='submit' name='activate' value='".(($premiumsettings['status'] == '') || ($premiumsettings['status'] == '0') ? $locale['MWP_a051'] : $locale['MWP_a052'])."' class='button' />";
	echo "</form>";
} else {
	echo "<center><font color='red'>".$locale['MWP_a066'] .$score_settings['set_units']."!</font></center>";
	if (file_exists(INFUSIONS."mw_buy_points_panel/mw_buy_points.php")) {
	echo "<center><br /><a class='button' title='".$score_settings['set_units']." kaufen' href='".INFUSIONS."mw_buy_points_panel/mw_buy_points.php'><strong>".$score_settings['set_units'].$locale['MWP_a056']." </strong></a></center>";
	}
}
