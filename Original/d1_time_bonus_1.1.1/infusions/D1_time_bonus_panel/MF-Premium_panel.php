...
//Überfällige aktivierungen Deaktivieren
//PLATIN-ACCOUNT
...
mysql_query("UPDATE ".DB_USERS." SET  `user_groups` =  '".str_replace(".".$gruppe['premium_group']."", "", $userdata['user_groups'])."' WHERE  `user_id` = ".mysql_real_escape_string($user_id).";");
//mysql_query("UPDATE ".DB_mfp_premium." SET status='wait' WHERE bis < $ablauf");
}
...
mysql_query("INSERT INTO ".DB_mfp_premium." (`user`, `status`, `seit`, `bis`, `code`) VALUES ('".$userdata['user_id']."', 'wait', '".time()."', '".time()."', '".$artikel_nr."-".$userdata['user_id']."');");
}
//Premium Ende
//Überfällige aktivierungen Ende
