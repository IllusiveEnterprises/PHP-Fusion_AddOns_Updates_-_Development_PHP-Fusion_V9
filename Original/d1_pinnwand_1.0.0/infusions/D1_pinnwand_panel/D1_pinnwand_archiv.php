...
if (file_exists(INFUSIONS."D1_pinnwand_panel/pinnwand_includes.php")) {
	include_once INFUSIONS."D1_pinnwand_panel/pinnwand_includes.php";
...
$rows = dbcount("(pinnwand_id)", " ".$db_prefix."d1_pinnwand_system","pinnwand_date");
include_once INFUSIONS."D1_pinnwand_panel/includes/comments_include.php";
$result = dbquery("SELECT * FROM ".$db_prefix."d1_pinnwand_system bs
LEFT JOIN ".DB_PREFIX."users bu ON bs.pinnwand_writer=bu.user_id
ORDER BY pinnwand_id DESC LIMIT ".$_GET['rowstart'].",$limit");
...
$item_idx = $data['pinnwand_id'];
		...
		if (iMEMBER) {
			...
			echo "<input type='submit' name='post_commentx' value='Kommentieren' class='button' />\n";
			echo "</div>\n</form>\n";
		} else {
			echo $locale['c105']."\n";
		}
...
if ($comment_messagex != "") {
					...
					if (!flood_control("comment_datestamp", DB_COMMENTS, "comment_ip='".USER_IP."'")) {
						$result = dbquery("INSERT INTO ".DB_COMMENTS." (comment_item_id, comment_type, comment_name, comment_message, comment_datestamp, comment_ip, comment_hidden, message_send) VALUES ('$comment_iid', 'BS', '$userid', '$comment_messagex', '".time()."', '".USER_IP."', '0', '1')");
