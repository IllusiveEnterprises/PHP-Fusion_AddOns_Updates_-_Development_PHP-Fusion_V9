...
function showcomments($ctype, $cdb, $ccol, $cid) {
	...
	$d1pwbid1 = dbarray(dbquery("SELECT pinnwand_writer FROM ".DB_PREFIX."d1_pinnwand_system WHERE pinnwand_id='$cid'"));
	//$d1pwbid2 = dbarray(dbquery("SELECT user_name FROM ".DB_USERS." WHERE user_id='".$d1pwbid1['pinnwand_writer']."'"));
	//echo $d1pwbid2['user_name'];
	$d1pwss = dbarray(dbquery("SELECT pinnwand_pn FROM ".DB_PREFIX."d1_pinnwand_system_settings"));
	if ($d1pwss['pinnwand_pn'] == '1') {
	//-----USERS PN NOTIFICATION-----//
	$commessage = "".$userdata['user_name']." hat diesen Pinnwand eintrag kommentiert --> [url=".$settings['siteurl']."infusions/D1_pinnwand_panel/D1_pinnwand.php?page=pinnwand_id&id=$cid]gehe zu Post[/url]<br><br>[b][u]Vorschau:[/u][/b]<br>$comment_message";
	...
	//-----USERS PN NOTIFICATION-----//
		...
		if (iMEMBER || $settings['guestposts'] == "1") {
			...
			echo "<div align='center' class='tbl'>\n";
			echo "<textarea name='comment_message' cols='80' rows='1' class='textbox' style='width:360px'>".$comment_message."</textarea><br />\n";
		...
		*/
		...
		$c_rows = dbcount("(comment_id)", DB_COMMENTS, "comment_item_id='$cid' AND comment_type='BS' AND comment_hidden='0'");
		if ($c_rows > '4') {
		$rowrow = $c_rows-4;
		} else {
		$rowrow = 0;
		...
		$result = dbquery(
			"SELECT tcm.comment_id, tcm.comment_item_id, tcm.comment_name, tcm.comment_message, tcm.comment_datestamp, tcu.user_name, tcu.user_status, tcu.user_avatar
			FROM ".DB_COMMENTS." tcm
			LEFT JOIN ".DB_USERS." tcu ON tcm.comment_name=tcu.user_id
			WHERE comment_item_id='$cid' AND comment_type='$ctype' AND comment_hidden='0'
			ORDER BY comment_datestamp ASC LIMIT $rowrow,4"
		);
		if (dbrows($result)) {
			$i = $_GET['c_start']+1;
			if ($c_rows > 10) {
			...
			while ($data = dbarray($result)) {
				...
				if ((iADMIN && checkrights("C")) || (iMEMBER && $data['comment_name'] == $userdata['user_id'] && isset($data['user_name']))) {
					...
					echo "<a href='".INFUSIONS."D1_pinnwand_panel/D1_pinnwand.php?page=pinnwand_id&amp;id=".$data['comment_item_id']."&amp;c_action=delete&amp;comment_id=".$data['comment_id']."'>".$locale['c109']."</a>\n</div>\n";
				}
				if ($data['user_avatar'] && file_exists(IMAGES."avatars/".$data['user_avatar']) && $data['user_status']!=6 && $data['user_status']!=5) {
					echo"<img style='vertical-align: middle;' src='".IMAGES."avatars/".$data['user_avatar']."' width='16' height='16' alt='".$data['comment_name']."' />&nbsp;";
				} else {
					echo"<img style='vertical-align: middle;' src='".IMAGES."avatars/noavatar50.png' width='16' height='16' alt='".$data['comment_name']."' />&nbsp;";
				}
