...
+----------------------------------------------------*/
$result = dbquery("SHOW TABLES LIKE '".DB_PREFIX."d1_pinnwand_system_settings'");
if (dbrows($result)) {
...
} elseif (d1pinnwandSet("inf_name") == "" || d1pinnwandSet("inf_name") != md5("D1 Pinnwand") || d1pinnwandSet("site_url") == "" || d1pinnwandSet("site_url") != md5("D1 Pinnwand".$settings['siteurl'])) {
} else {
...
$trim = 20; 
// Limit
$limit = "".$d1pwss['pinnwand_userlimit']."";
...
if (isset($sunvotedbcheck)) {
	$result11 = dbquery("SELECT * FROM ".DB_PREFIX."sun_vote WHERE vote_item_id='".$data['pinnwand_id']."' AND vote_type ='KB' ");
...
echo '&nbsp;';
	$result_vote = dbquery("SELECT vote_user_id, vote_item_id FROM ".DB_PREFIX."sun_vote
...
echo "<div style='float: right;'><a href='http://www.deeone.de' target='_blank' title='D1 Pinnwand ".$locale['D1PW_vers']." &copy;2013 by DeeoNe'><span class='small'>&copy;</span></a></div>";
