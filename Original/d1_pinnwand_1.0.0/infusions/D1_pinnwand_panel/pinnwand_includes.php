...
if ($type == 'e') {
	...
	//echo "<form name='inputform' method='post' action='".INFUSIONS."D1_pinnwand_panel/D1_pinnwand_wall.php' >";
	echo "<form name='inputform' method='post' action='".FUSION_SELF."' >";
}
?>
    <table cellpadding='0' cellspacing='1' width='90%' class='center tbl-border'>
...
$d1pwsic = dbarray(dbquery("SELECT * FROM ".DB_PREFIX."d1_pinnwand_system_settings"));
if ($d1pwsic['pinnwand_sonst1'] == '0') {
$zufzahl = rand(1,1);
$wmdgtext[1] = "Was machst du gerade, ".$userdata['user_name']."?"; //Standart TEXT!!! bei Ausgeschaltetem Zufallstext
} else {
require_once INFUSIONS."D1_pinnwand_panel/includes/pinnwand_zufallstext.php";
}

if(isset($_POST["pinnwand_text"])) {
$posttext = $_POST['pinnwand_text'];
$textclass = "textbox";
} else {
$posttext = $wmdgtext[$zufzahl];
$textclass = "textbox default-value";
}


	if ($type == "c") {
	echo "<textarea name='pinnwand_text' class='$textclass' cols='80' rows='2'>".$posttext."</textarea>";
	} else {
	echo "<textarea name='pinnwand_text' class='textbox' cols='80' rows='2'>".$pinnwand_text."</textarea>";
	} 
	?>
</center></td>
    </tr>
...
<td class='tbl1' style='text-align:right;' width='10%'>
 <input type='submit' name='<?php echo ($type == "c" ? "save" : "edit"); ?>' value='<?php echo ($type == "c" ? $locale['TIBLCB006'] : $locale['TIBLEB006']); ?>' class='button' />
    ...
    <?php 
		echo ($type == 'e' ? "<tr>
    	<td class='tbl2' style='text-align:center;'><font color='#FF0000'><b>".$locale['TIBLEB009'].":</b> <input type='checkbox' name='delete' value='1' /></font></td>
...
echo "<center><b>--- Posten nur f&uuml;r Mitglieder ---</b></center>";
}
...
global $pinnwand_write, $pinnwand_subject, $pinnwand_text, $locale, $prfx, $settings;
	...
	if (isset($_POST['save'])) {
		...
		if ($d1pwss['pinnwand_scores'] == '1') {
		score_free("Pinwannd Post", "PWP", "".$d1pwss['pinnwand_scoresh']."", 20, "P", 0, 0);
		}
...
$tinewview = dbquery("UPDATE ".DB_PREFIX."d1_pinnwand_system SET pinnwand_views=pinnwand_views+1 WHERE pinnwand_id='$id'");
	...
	if ($tipinnwand['user_avatar']) {
		...
		<a href='".BASEDIR."profile.php?lookup=".$tipinnwand['pinnwand_writer']."'><img src='".BASEDIR."images/avatars/".$tipinnwand['user_avatar']."' /></a>
		<center><a href='".BASEDIR."profile.php?lookup=".$tipinnwand['pinnwand_writer']."'>".$tipinnwand['user_name']."</a></center></div>";
	} else {
		echo "<div id='bild' align='right' style='margin:5px; width:100px; float:right;'>
		<a href='".BASEDIR."profile.php?lookup=".$tipinnwand['pinnwand_writer']."'><img src='".BASEDIR."images/avatars/noavatar100.png' /></a>
		<center><a href='".BASEDIR."profile.php?lookup=".$tipinnwand['pinnwand_writer']."'>".$tipinnwand['user_name']."</a></center></div>";
	}
...
<?php
echo "<hr><center><a class='button' href='".INFUSIONS."D1_pinnwand_panel/D1_pinnwand.php' title='Was machst du gerade Archiv'>&middot; zur&uuml;ck zur Pinnwand &middot;</a></center>";
?>
    <?php
	closetable();
	//include INCLUDES."comments_include.php";
	include INFUSIONS."D1_pinnwand_panel/includes/comments_include2.php";
...
global $locale, $pinnwand_text, $pinnwand_subject, $prfx, $settings;
	if (isset($_POST['edit'])) {
		...
		if ($_POST['delete'] == 1) {
			...
			header("Location: ".INFUSIONS."D1_pinnwand_panel/D1_pinnwand.php");
		} else {
			$result = dbquery("UPDATE ".DB_PREFIX."d1_pinnwand_system SET pinnwand_text='$pinnwand_text' WHERE pinnwand_id='$id'");
			header("Location: ".INFUSIONS."D1_pinnwand_panel/D1_pinnwand.php");
		}
	} else if (isset($_POST['preview'])) {
		$pinnwand_subject = stripinput($_POST['pinnwand_subject']);
